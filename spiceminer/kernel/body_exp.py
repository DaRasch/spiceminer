...
class _BodyMeta(type):
    ...
    def __init__(cls, *args):
        super(cls.__class__, cls).__init__(*args)
        if not hasattr(cls, 'LOADED'):
            cls.LOADED = set()

    def _make(cls, id_):
        if _ID_COUNTER[id_] == 0:
            ...
            #    super(body.__class__, body).LOADED.add(body)
            _ID_MAP[id_] = body
        _ID_COUNTER[id_] += 1
    ...
    def _delete(cls, id_):
        _ID_COUNTER[id_] -= 1
        body = cls(id_)
        count = _ID_COUNTER[id_]
        if count == 0:
            cls.LOADED.remove(body)
            #while True:
            #    try:
            #        body.__class__.LOADED.remove(body)
            #        body = super(body.__class__, body)
...
class Body(object):
    ...
    *classattribute* LOADED: set of body
    ...
    @property
    def times_pos(self):
        return shared.TIMEWINDOWS_POS[self.id]

    @property
    def times_rot(self):
        return shared.TIMEWINDOWS_ROT[self.id]
